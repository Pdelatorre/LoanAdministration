...
All commands are run from the `LoanAdministration_claude/` folder in PowerShell on Windows.

> **PowerShell note:** Line continuation uses a backtick (`` ` ``) instead of `\`. Descriptions containing a `$` sign must use single quotes (e.g. `'PIK elected, $1,666.67 capitalized'`) to prevent PowerShell treating it as a variable.
